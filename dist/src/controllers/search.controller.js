...
const tokenize = (value) => value
    ...
    .map(v => v.trim())
    .filter(Boolean);
const scoreAnime = (keyword, title, id) => {
    ...
    if (normalizedTitle.includes(normalizedKeyword))
        score += 500;
    if (normalizedId.includes(normalizedKeyword))
        score += 400;
...
const filterAndRank = (keyword, payload) => {
...
const scrapeSearchFallback = async (keyword, page) => {
