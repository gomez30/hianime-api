...
const homepageController = async () => {
    ...
    const hasAnyData = parsed.spotlight.length > 0 ||
    ...
    console.log(`Homepage scrape timings: total=${totalDurationMs}ms, upstream=${result.meta.durationMs}ms, parse=${parseDurationMs}ms`);
    if (!hasAnyData) {
        const staleData = getCachedHomepage();
        if (staleData) {
