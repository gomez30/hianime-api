...
export function extractAnimeInternalIdCandidatesFromHtml(html) {
    if (!html)
        return [];
    ...
    $('.film-poster.item-qtip[data-id], .film-poster[data-id], .anisc-poster[data-id], [data-id][class*="item-qtip"]').each((_, el) => {
        const id = $(el).attr('data-id');
        if (id)
            addCandidate(id, 60);
    });
